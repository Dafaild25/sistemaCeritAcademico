...
function vistaAsitencias(clase_id){
  ...
  const periodoAsistencia = document.getElementById('periodoAsistencia'); // 
  
  if (periodoAsistencia) { // Verifica que periodoTrabajo no sea null o undefined
      const periodo_id = periodoAsistencia.getAttribute('data-periodoAsistencia');
    ...
    .then((data) => {
      if (data.status === "success") {
        alert("Asistencia guardada exitosamente.");
         // Ocultar la columna de asistencia y su encabezado después de guardar
         const thAsistencia = document.getElementById("btn-asis");
         if (thAsistencia) {
           thAsistencia.style.display = "none"; // Oculta el encabezado de la columna
         }
 
         const columnasAsistencia = document.querySelectorAll(".asistencia-col");
         columnasAsistencia.forEach(col => {
           col.style.display = "none"; // Oculta las celdas de la columna de asistencia
         });
        loadCalendar();
...
function marcarDiasConAsistencia(diasConAsistencia) {
  diasConAsistencia.forEach((dia) => {
    ...
    if (diaElemento) {
      diaElemento.classList.add("con-asistencia");
    }
  });
}
...
function cargarReporte(claseId) {
  const contenedor = document.getElementById('contenedor-trabajo-clase');
