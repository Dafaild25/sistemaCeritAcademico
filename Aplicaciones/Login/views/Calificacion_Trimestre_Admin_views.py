...
def vistaCalificacionTrimestreAdmin(request, trimestre_id, clase_id):
    try:
        ...
        return render(request, 'Reportes/reportesTrimestrales.html', context)
    
    except Exception as e:
        print(f"Error en vistaCalificacionTrimestre: {e}")
        return HttpResponseServerError("Error en el servidor.")
    
def obtenerSubPromedioUnidadAdmin(request, unidad_id, matricula_id, trimestre_id,clase_id):
    try:
...
def calificacionExamenTrimestralAdmin(request):
...
def obtenerCalificacionExamenTrimestralAdmin(request, matricula_id, examen_id, clase_id):
    try:
...
def obtenerPromedioTrimestralAdmin(request, matricula_id, trimestre_id, clase_id):
    try:
...
def obtenerObservacionExamenAdmin(request, calificacion_id):
    try:
...
def editarObservacionExamenAdmin(request, calificacion_id):
    if request.method == 'POST':
        try:
            calificacion = CalificacionExamen.objects.get(id=calificacion_id)
